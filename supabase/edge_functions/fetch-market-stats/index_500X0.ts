...
serve(async (req) => {
  ...
  try {
    ...
    for (const categoryCode of CATEGORY_CODES) {
      const items = await fetchAllItemsForCategory(categoryCode, apiAuthHeader);
      for (const item of items) {
        const result = await processSingleItem(item, supabaseClient, apiAuthHeader, categoryCode);
        if (result) {
          totalItemsProcessed++;
        }
